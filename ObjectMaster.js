...
const pkmnNamesIdGreater99 = pokémon
    .filter((p) => p.id > 99)
    .map((p) => p.name)
...
const pkmnTypesFlyingSecond = pokémon
    .filter((p) => p.types[1] == "flying")
    .map((p) => p.types[0])
    console.log(pkmnTypesFlyingSecond);

// a count of the number of pokémon that are "normal" type

    const pkmnNormal = pokémon
        .filter((p) => p.types.includes("normal"))
        const pkmnNormalCount = pkmnNormal.length
